from connect.serializers import UserSerializer, FriendSerializer, FriendMessagesSerializer, ForgotPasswordSerializer
...
class ForgotPasswordEmail(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            forgot_password = ForgotPassword(email=request.data["email"], password_reset_token=password_reset, user=user)
            forgot_password.save()

            forgot_password_link = f"{settings.VITE}/login/forgot-password/password-reset/{password_reset}"
...
class PasswordReset(generics.CreateAPIView):
    ...
    def post(self, request):
        #gets forgot password object from database by password reset token
